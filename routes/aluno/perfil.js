...
router.get('/aluno/perfil', isAuthenticated, isAluno, async (req, res) => {
    try {
        const db = getDB();
...
router.post('/aluno/perfil/alterar-password', isAuthenticated, isAluno, async (req, res) => {
    try {
        const db = getDB();
        ...
        if (!await bcrypt.compare(pwd_atual, user.pwd)) {
            return res.send(`<script>alert('Password atual incorreta!'); window.location.href='/aluno/perfil';</script>`);
        }
        if (pwd_nova !== pwd_confirmar) {
            return res.send(`<script>alert('As passwords não coincidem!'); window.location.href='/aluno/perfil';</script>`);
        }
        if (pwd_nova.length < 4) {
            return res.send(`<script>alert('A password deve ter pelo menos 4 caracteres!'); window.location.href='/aluno/perfil';</script>`);
        }
        ...
        res.send(`<script>alert('Password alterada com sucesso!'); window.location.href='/aluno/perfil';</script>`);
    } catch (error) {
        res.send(`<script>alert('Erro ao alterar password'); window.location.href='/aluno/perfil';</script>`);
    }
});
...
function generatePerfilHTML(user, login) {
    ...
    <head>
        ...
        <style>
            .profile-header { background:linear-gradient(135deg,#28a745 0%,#20c997 100%); color:white; padding:40px; text-align:center; border-radius:15px; margin-bottom:30px; }
            .profile-avatar { width:100px; height:100px; background:white; border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center; font-size:3em; color:#28a745; }
    ...
    <body>
        <div class="container">
            <div class="header"><h1>👤 Meu Perfil (Aluno)</h1><p>${login}</p></div>
            <div class="nav">
            ...
            <div class="content">
                <div class="profile-header">
                    ...
                    <h2>${login}</h2>
                    <p><span class="badge badge-aluno">ALUNO</span></p>
                </div>
                <div class="profile-info">
                    <div class="info-item"><div class="info-label">Login</div><div class="info-value">${login}</div></div>
                    <div class="info-item"><div class="info-label">Tipo</div><div class="info-value"><span class="badge badge-aluno">ALUNO</span></div></div>
                </div>
                <div class="card"><h2>🔒 Alterar Password</h2>
                    <form method="POST" action="/aluno/perfil/alterar-password" style="max-width:500px;margin:0 auto">
